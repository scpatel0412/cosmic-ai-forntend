import { Link } from "react-router-dom";
import moment from "moment";
interface SideNavInterface {
    conv_data: any;
}

function Sidenav({ conv_data }: SideNavInterface) {
    return (
        <aside
            id="default-sidebar"
            className="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0"
            aria-label="Sidebar"
        >
            <div className="flex flex-col h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
                <ul className="space-y-2 font-medium">
                    {conv_data &&
                        conv_data.data.map((item: any) => {
                            return (
                                <li key={item.id}>
                                    <Link
                                        to={`/chat/${item.id}`}
                                        className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"
                                    >
                                        <svg
                                            className="w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
                                            aria-hidden="true"
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="currentColor"
                                            viewBox="0 0 22 21"
                                        >
                                            <path d="M16.975 11H10V4.025a1 1 0 0 0-1.066-.998 8.5 8.5 0 1 0 9.039 9.039.999.999 0 0 0-1-1.066h.002Z" />
                                            <path d="M12.5 0c-.157 0-.311.01-.565.027A1 1 0 0 0 11 1.02V10h8.975a1 1 0 0 0 1-.935c.013-.188.028-.374.028-.565A8.51 8.51 0 0 0 12.5 0Z" />
                                        </svg>
                                        <span className="ms-3 flex space-between">
                                            <span>{item.summary}</span>
                                            <span className="bold" style={{ marginLeft: '55px' }}>
                                                {moment(item.created_at).format("YYYY-MM-DD")}
                                            </span>
                                        </span>
                                    </Link>
                                </li>
                            );
                        })}
                </ul>
                <div className="mt-auto p-4 bg-gray-100 dark:bg-gray-700 rounded-tl-lg rounded-br-lg">
                    <div className="flex items-center">
                        <img
                            src='https://unsplash.com/photos/ggcEFpKzflQ/download?ixid=M3wxMjA3fDB8MXxhbGx8fHx8fHx8fHwxNzQwMzgyNjA0fA&force=true&w=640'
                            alt="Profile Avatar"
                            className="w-16 h-20 rounded-xl object-cover"
                        />
                        <div className="ml-3">
                            <p className="text-sm font-semibold text-gray-900 dark:text-white">
                                Ana de Armas
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </aside>
    );
}

export default Sidenav;
